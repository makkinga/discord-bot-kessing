...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('snow')
        .setDescription(
            'Split your CRYSTAL between 10 active members in the last 100 messages',
        )
        .addNumberOption((option) =>
            option
                .setRequired(true)
                .setName('amount')
                .setDescription('Enter the amount to tip'),
        )
        .addStringOption((option) =>
            option
                .setRequired(false)
                .setName('role')
                .setDescription('Select a role to snow on')
                .addChoices(
    ...
    async execute(interaction) {
        ...
        const token = 'CRYSTAL'
        const role =
            config.rain_roles[interaction.options.getString('role')] ?? null
