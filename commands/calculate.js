...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setReqired(true)
                .setName('in')
                .setDescription('Select a currency')
                .addChoices(
                    { name: '$', value: 'usd' },
                    { name: '€', value: 'eur' },
                    { name: '£', value: 'gbp' },
    ...
    async execute(interaction) {
        ...
        const tokenIcons = {
            ...
            CRYSTAL:
                'https://storageapi2.fleek.co/ed2319ff-1320-4572-a9c4-278c4d80b634-bucket/dfk/logo_crystal.png',
        ...
        const rates = await axios(
            `https://api.freecurrencyapi.com/v1/latest?apikey=${process.env.CURRENCY_API_KEY}`,
        )
        ...
        const embed = new EmbedBuilder()
            .setAuthor({
                name: tokenSymbols[token],
                iconURL: tokenIcons[token],
            })
            .setDescription(
                `${amount} ${tokenSymbols[token]} ≈ ${currencySymbols[currency].replace(':price', value)}`,
            )
